...
return ret},sha1:function(str){return CryptoJS.SHA1(str).toString()},sha256:function(str){return CryptoJS.SHA256(str).toString()},hmacSha256:function(str,key){return CryptoJS.HmacSHA256(str,key).toString()}}
...
return{i18n:{CSRF_VALUE_NOT_SET:'CSRF cookie not set ; request has been stopped.'},replaceLinksWithPOSTRequests:function(){_autoinit()},autoReplaceLinksWithPOSTRequests:function(){nettools.jscore.registerOnLoadCallback(_autoinit)},setCSRFCookieName:function(name){_csrf_cookiename=name},getCSRFCookieName:function(){return _csrf_cookiename},setCSRFSubmittedValueName:function(name){_csrf_submittedvaluename=name},getCSRFSubmittedValueName:function(){return _csrf_submittedvaluename},setCSRFHashedCookieValue:function(hashedvalue){_csrf_hashed_cookie=hashedvalue},addCSRFSubmittedValueHiddenInput:function(form){var input=document.createElement('input');input.type='hidden';input.name=_csrf_submittedvaluename;input.value=_getCSRFCookie();form.appendChild(input)},addCSRFSubmittedValue:function(postData){if(window.FormData&&(postData instanceof FormData)){postData.set(_csrf_submittedvaluename,_getCSRFCookie());return postData}else{if(typeof postData=='object'){postData[_csrf_submittedvaluename]=_getCSRFCookie();return postData}else return nettools.jscore.appendToUrl(postData,_csrf_submittedvaluename+'='+_getCSRFCookie())}},addCSRFHashedValue:function(url){return nettools.jscore.appendToUrl(url,_csrf_submittedvaluename+'='+_getCSRFHashedCookie())},post:function(url,postData){var data=_addCSRFValue(postData);nettools.jscore.RequestHelper.post(url,data)},sendXmlHTTPRequest:function(url,callback,postData){return nettools.jscore.xmlhttp.sendRequest(url,callback,_addCSRFValue(postData))},sendXmlHTTPRequestPromise:function(url,postData){try
